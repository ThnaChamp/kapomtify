...
export default function ChartPage() {
  ...
  useEffect(() => {
    const delay = setTimeout(() => fetchCharts(), 500);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ chart_code: "", chart_name: "", description: "", chart_date: "" });
        fetchCharts();
      } else {
    ...
    <div className="flex flex-col gap-5 p-0 text-[#e0e0e0]">
            ...
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 z-10 bg-[#252525]">
                <tr className="text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-[#333]">
                  ...
                  <th className="px-6 py-4">Total Songs</th>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#282828] p-8 rounded-xl border border-[#333] w-full max-w-lg shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-white">Create Chart</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-white text-xl">
                x
              </button>
            </div>
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 flex justify-end gap-3 mt-4 border-t border-[#333] pt-4">
                <button
                  type="button"
